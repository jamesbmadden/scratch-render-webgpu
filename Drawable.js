...
export default class Drawable {
    ...
    _buildPipeline(renderer) {
        ...
        if (renderer._device === undefined)
            throw "Renderer is missing the device.";
        ...
        if (this.properties.skinId !== -1) {
            this.texture = (_a = renderer.skins[this.properties.skinId]) === null || _a === void 0 ? void 0 : _a.texture;
        }
        else {
     ...
     */
    writeToUniforms(renderer) {
        // make sure renderer has a device
        if (renderer._device === undefined)
            throw "Renderer is missing the device.";
        if (this.uniformBuffer === undefined)
            throw "Uniform buffer is missing";
        // convert properties object to uniforms
        const uniforms = this._getUniforms();
        renderer._device.queue.writeBuffer(this.uniformBuffer, 0, uniforms.buffer, uniforms.byteOffset, uniforms.byteLength);
    }
}
